...
class RecordBrowserPage(private val properties: GitHubProperties) {
        ...
        layout("기록 찾기", actor, buildString {
            ...
            if (page == null) {
                ...
                page.items.forEach { record ->
                    val recordUrl = recordUrl(record.id, searchUrl)
    ...
    fun record(actor: ActorIdentity, record: ChangeRecord, searchUrl: String? = null): String = layout(record.title, actor, buildString {
        ...
        if (record.targetRevision != null) append("<a class=\"button secondary\" href=\"${html(recordUrl(record.id, searchUrl, "evidence"))}\">GitHub 코드와 비교</a>")
        append("</section>")
