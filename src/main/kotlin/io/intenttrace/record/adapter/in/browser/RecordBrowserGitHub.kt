...
internal fun RecordBrowserPage.github(actor: ActorIdentity, repository: String?, request: GitHubRequestContext?,
    actions: GitHubActionsResults?): String = layout("GitHub 자료", actor, buildString {
    append("<header class=\"page-heading\"><h1>GitHub 자료</h1><p>이슈·PR에서 초안 내용을 가져오거나 이미 실행된 CI 결과를 확인하세요.</p></header>")
    ...
    request?.let {
        ...
        append("<label>초안에 사용할 요청 요약<textarea rows=\"8\" readonly>${html(it.requestSummary)}</textarea></label>")
        append("<p class=\"muted\">가져온 내용을 검토한 뒤 Agent에서 초안을 만드세요. 작성자 확인과 공개는 별도로 진행합니다.</p>")
        if (it.truncated) append("<p class=\"notice\">입력 길이에 맞춰 일부만 가져왔습니다. 원문에서 빠진 내용을 확인하세요.</p>")
    ...
    append("""<form action="/records/github" class="search-form" method="get">
        <label>저장소<input name="repositoryKey" value="${html(repository.orEmpty())}" placeholder="owner/repository" required></label>
        <label>전체 커밋 ID<input name="revision" value="${html(actions?.revision.orEmpty())}" required></label>
        <button>CI 결과 조회</button></form>""")
    actions?.let { result ->
        append("<p class=\"muted\">GitHub가 이 커밋에 연결한 실행 결과입니다. 테스트가 실행된 코드 전체나 로컬 검증을 증명하지 않습니다.</p>")
        if (result.items.isEmpty()) append("<p class=\"empty\">조회된 실행 결과가 없습니다. 테스트 통과를 뜻하지 않습니다.</p>")
        append("<ul class=\"records\">")
        result.items.forEach { run ->
            val status = if (run.status != "completed") "진행 상태: ${run.status}" else when (run.conclusion) {
                "success" -> "성공"; "failure" -> "실패"; "cancelled" -> "취소"; "skipped" -> "건너뜀"
                "timed_out" -> "시간 초과"; "neutral" -> "참고"; else -> "결과: ${run.conclusion ?: "확인 중"}"
            }
            append("<li><div><span class=\"status\">${html(status)}</span><h3><a href=\"${html(run.url)}\">${html(run.name.ifBlank { "워크플로 #${run.id}" })}</a></h3>")
            append("<p>실행 시도 ${run.attempt} · ${html(run.event)}</p><p class=\"muted\">${run.startedAt?.let { "시작 ${stamp(it)} · " }.orEmpty()}최근 갱신 ${stamp(run.updatedAt)}</p></div></li>")
        }
