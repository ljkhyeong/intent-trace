...
internal fun RecordBrowserPage.github(actor: ActorIdentity, repository: String?, request: GitHubRequestContext?,
    actions: GitHubActionsResults?, actionsPage: Int = 1): String = layout("이슈·PR·CI", actor, buildString {
    ...
    request?.let {
        ...
        if (it.kind == GitHubRequestKind.PULL_REQUEST) {
            append("<p><a class=\"button secondary\" href=\"${html(url("/records/pull-requests", "repositoryKey" to it.repositoryKey, "pullNumber" to it.number.toString()))}\">이 PR의 변경 기록 보기</a></p>")
        }
    ...
    actions?.let { result ->
        fun pageUrl(page: Int) = html(url("/records/github", "repositoryKey" to result.repositoryKey,
        ...
        if (result.items.isEmpty()) append("<p class=\"empty\">${if (actionsPage == 1) "조회된 CI 실행이 없습니다." else "이 페이지에 CI 실행이 없습니다. 이전 페이지에서 확인하세요."}</p>")
        ...
        append("</ul><p class=\"muted\">${actionsPage}페이지 · ${result.items.size}건 · 조회 ${stamp(result.fetchedAt)}</p>")
        append("<nav class=\"pagination\" aria-label=\"CI 결과 페이지\">")
        if (actionsPage > 1) append("<a class=\"button secondary\" href=\"${pageUrl(actionsPage - 1)}\">이전 실행 결과</a>")
        result.nextPage?.let { append("<a class=\"button secondary\" href=\"${pageUrl(it)}\">다음 실행 결과</a>") }
        append("<a class=\"button secondary\" href=\"${pageUrl(actionsPage)}\">결과 새로고침</a></nav>")
