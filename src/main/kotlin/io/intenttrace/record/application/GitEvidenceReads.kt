...
internal class GitEvidenceReads(private val repository: GitHubRepository, private val gateway: GitEvidenceGateway) {
    ...
    fun snapshot(revision: String) = read(snapshots, revision) { gateway.snapshot(repository, revision) }
    fun blob(sha: String) = read(blobs, sha) { gateway.blob(repository, sha) }
    fun isAncestor(from: String, to: String) = read(ancestry, "$from:$to") { gateway.isAncestor(repository, from, to) }
