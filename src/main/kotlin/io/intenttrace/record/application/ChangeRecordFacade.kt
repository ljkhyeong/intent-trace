...
class ChangeRecordFacade(
    ...
    fun create(command: CreateChangeRecordCommand): ChangeRecord {
        repository.findByRequestId(command.requestId)?.let { return it }
    ...
    fun supersede(command: SupersedeChangeRecordCommand): ChangeRecord {
        ...
        return repository.update(current.supersede(replacement), command.expectedVersion)
    }
    ...
    private fun validateCreate(command: CreateChangeRecordCommand) {
        ...
        require(command.createdBy.isNotBlank()) { "작성자는 비어 있을 수 없습니다." }
