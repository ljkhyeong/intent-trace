...
1. 실제 IntelliJ에서 새 기록함의 필터·페이지·상세·브라우저 이동 동선을 확인한다. 2026-08-30 확인 시 설치된 플러그인은 0.7.0이었다. 개발용 `runIde`는 실행됐지만 화면 제어 도구가 Java 프로세스를 식별하지 못했다. 현재 IDE에 0.8.0-SNAPSHOT ZIP 설치·재시작 승인을 받은 뒤 이어서 확인해야 한다. 화면 조작 검증은 완료되지 않았다.
