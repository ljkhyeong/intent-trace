...
- Streamable HTTP MCP 도구 21개, 줄 조회의 최상위 `items` 객체 응답
...
0.9.0 이후 우선순위는 [후속 개선 검토](docs/reviews/2026-09-05-v0.9-follow-up-review.md)를 따른다. 새 GitHub 설치 token의 전체 제거, 요청 안의 중복 조회, 일부 후보 조회 실패와 게시 사전 점검을 우선 제안한다. 아직 구현하지 않았다.
...
- Zed Agent MCP 연결을 구현했다. 인라인 IDE 메뉴와 자동 기록 수집은 없다. 실제 Zed 앱이 작업 환경에 없어 앱 화면·사용자 GitHub 승인 완료는 확인하지 않았다.
- 연결 진단의 서버 게시 자격 증명은 설정 존재만 확인하며 실제 키·설치·Checks 쓰기 권한은 별도 확인이 필요하다.
- 게시 시도 이력 외의 감사 로그와 자동 보존 정책은 아직 구현하지 않았다.
- Micrometer 지표는 수집하지만 외부 수집기와 대시보드는 별도 연결해야 한다.

## 0.9.0 검증 결과

- 전체 서버 테스트: 79개 중 78개 통과, PostgreSQL 전용 1개는 기본 실행에서 제외.
- 별도 PostgreSQL 17에서 V8 원본 연결·PR 목록·backup/restore 왕복 통과.
- Zed 연결 입력·인증 실패 처리 3개 통과. 실제 Spring 서버와 stdio 중계 통합 테스트는 전체 서버 테스트에 포함.
- 플러그인과 사용·개발 스킬 검증 통과.
