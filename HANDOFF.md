...
- Streamable HTTP MCP 도구 23개, 줄 조회의 최상위 `items` 객체 응답
...
[0.10.0 후속 개선 검토](docs/reviews/2026-09-05-v0.10-follow-up-review.md)에서는 Zed 오류 분류·대기 시간 전달, 웹 파일·줄 조회와 코드 근거 확인, 웹 연결 관리, 비교의 세부 변경 표시, 기록 상태 변경 이력을 제안했다. Zed 중계기의 HTTP 401·429·502 정보 손실은 로컬에서 재현했으며 아직 수정하지 않았다. 우선순위는 오류 처리 → 웹 조회·연결 관리 순이다.
