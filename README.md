...
조회 거부나 자료 없음은 저장소·이슈·PR 주소와 GitHub App 읽기 권한을 확인하도록 안내합니다. 일시 장애와 호출 제한에는 `다시 조회`를 제공하며, 호출 제한은 안내된 대기 시간 뒤에 재시도합니다.
...
5. 저장소 파일을 선택하고 `Tools > IntentTrace 기록함 열기`에서 팀 공개 기록·내 비공개 기록·상태·현재 파일 필터를 고릅니다. 필요하면 검색어를 입력하고 `조회` 또는 Enter 키로 검색합니다. 기록을 선택하면 같은 상세 화면을 엽니다.
