---
name: intent-trace
description: AI가 작성하거나 수정한 코드의 요청·판단·검증을 IntentTrace 초안으로 기록하거나 기존 기록을 조회할 때 사용한다. 변경 의도 기록, 코드 설명, 팀 인수인계, PR 근거, 현재 줄 조회, 내 비공개 기록함과 파일별 과거 이력 조회에 적용한다.
---
...
- 커밋이나 기록 UUID를 모르거나 과거 이력을 찾을 때는 `list_change_records`를 사용한다. `repositoryKey`는 필수이며 파일 이력은 `path`에 정확한 저장소 상대 경로를 지정한다. 파일 이름 변경 전 경로는 자동 추정하지 않는다.
- 팀 공개 기록은 `scope=TEAM`(기본값), 내 비공개 초안·작성자 확인 기록은 `scope=MY_DRAFTS`로 조회한다. 작성자 ID는 입력하지 않는다. 팀 공유 요청에 내 비공개 기록을 섞지 않는다.
- `status`는 선택한 기록함 안에서만 좁힌다. `page`는 0부터, `size`는 기본 20·최대 50이며 추가 결과가 필요하고 `hasNext=true`일 때 다음 페이지를 읽는다. 한 페이지를 전체 검색 결과라고 표현하지 않는다.
- 목록은 요약이다. 내용을 설명할 기록은 `get_change_record`로 상세를 읽는다. `supersededBy`가 있으면 대체된 기록임을 밝히고, 최신 판단이 필요한 요청에서는 후속 기록을 따라간다.
- 현재 줄 조회 결과가 없어도 기록 전체가 없다고 단정하지 않는다. 파일의 과거 이력을 찾으면 현재 줄의 확정 근거가 아닌 다른 커밋의 기록으로 제시한다.
- 검증의 `current`는 기록 스냅샷과의 일치 여부다. 현재 편집 중인 코드가 검증됐다는 뜻으로 설명하지 않는다.
