...
- PR과 커밋을 함께 지정하면 조회한 PR HEAD와 비교해 `pull_request_revision=VERIFIED|FAILED`로 반환한다. 일치하지 않아도 지정한 커밋의 트리 읽기 등 나머지 진단은 계속한다. PR 조회 실패나 커밋 생략 시에는 비교 항목을 추가하지 않는다. 추가 GitHub 호출은 없다.
...
- Git 트리를 읽을 때 응답 크기 초과·불완전한 트리·지원하지 않는 객체가 발생하면 `git_tree_read=FAILED`와 해당 사유를 반환하고 나머지 진단을 계속한다. 이를 PR 번호·권한 오류로 안내하지 않는다. 웹·REST·MCP는 공통 사유 문구를 사용한다.
...
- 단일 인자가 `owner/repo` 형태이면 저장소로 처리한다. 주소를 생략한 점검은 `INTENT_TRACE_MCP_URL`, 없으면 기본 로컬 서버를 사용하고 직접 입력한 MCP 주소가 우선한다. 주소 검증과 토큰 전달 규칙은 동일하다.
- `check`는 정상 진단 응답의 항목 이름·상태와 서버가 제공한 `message`를 함께 출력한다. 실패 항목이 있어도 나머지 진단을 표시하고 종료 코드 1을 반환한다. HTTP 오류 원문은 기존대로 버린다.
- 알 수 없는 CLI 명령은 입력 원문 없이 안내하고 종료 코드 1로 실패한다. 인자가 없거나 `--help`·`-h`를 지정하면 종료 코드 0으로 도움말을 표시한다. 명령별 도움말은 주소·세션 확인과 설정 파일 접근 전에 처리한다. `launch`의 인자는 기존대로 Zed에 전달한다.
- `--version`·`-V`는 주소·세션 없이 연결 도구의 버전을 출력한다. CLI와 MCP 중계기의 양쪽 연결 정보는 같은 `package.json` 버전을 사용한다. 서버 버전을 대신 표시하지 않는다.
...
새 설정 파일은 제한된 권한으로 만들고 기존 파일은 권한을 유지한다. 임시 파일 작성 후 교체하며 직전 읽은 내용과 달라졌으면 재실행을 요구한다. 토큰은 설정 파일에 저장하지 않는다. 실제 Zed 앱 검증과 로컬 테스트 응답의 사용 범위는 `docs/clients/zed.md`에 남긴다.

`unconfigure`는 같은 미리보기·`--apply`·파일 교체 절차로 `context_servers.intent-trace`만 제거한다. 기본 속성 삭제가 인접 주석까지 지우므로 JSONC 라이브러리의 구문 트리와 토큰 스캐너로 속성과 구분 쉼표만 편집한다. 주변 주석·다른 연결·파일 권한을 유지한다. 항목이나 설정 파일이 없으면 아무 파일도 쓰지 않는다. 설정 명령의 옵션은 Node.js 기본 파서로 처리하며 제거에는 MCP 주소·세션·서버 호출이 필요 없다. 패키지 삭제와 서버 세션 폐기는 수행하지 않는다.
