...
- `scripts/package-zed.mjs`는 고정 의존성을 설치한 `clients/zed`에서 실행 파일·숨긴 세션 입력 도구·사용 안내와 Node 의존성을 묶은 `.tgz` 및 SHA-256 파일을 만든다. 저장소 전체·테스트·사용자 설정은 포함하지 않는다. 생성은 macOS·Linux에서 지원한다.
