...
- 열린 결과 창의 후속 조회는 최초 조회 서버를 유지한다. 기록함의 검색·페이지 이동·새로고침과 상세·원본·대체·파일 이력에 같은 주소를 전달하며, 요청마다 해당 서버의 세션을 다시 읽는다. 창에 토큰을 보관하지 않는다.
- 현재 줄 조회 결과는 빈 목록도 같은 창으로 표시한다. `웹에서 줄 이동·이름 변경 찾기`는 조회 당시 서버의 `/records/history`에 저장소·전체 커밋·파일·줄을 URL 인코딩해 전달한다. 설정이나 HEAD를 다시 읽지 않고 세션 토큰도 전달하지 않는다. 웹의 별도 로그인·권한 확인과 기존 코드 비교를 사용하며, 버튼을 누르기 전에는 웹 조회를 실행하지 않는다.
