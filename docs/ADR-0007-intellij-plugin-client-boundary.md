...
- `429`는 조회·상태 확인·세션 해제에 같은 안내를 사용한다. 서버가 초 단위로 반환한 `Retry-After`의 0 이상 정수를 표시하고 누락·잘못된 값은 대기 시간 미확인으로 안내한다. 오류 본문이나 헤더 원문을 표시하지 않으며 자동 대기·재시도하지 않는다. 세션 해제도 실패로 전달해 PasswordSafe 삭제 전에 중단한다.
