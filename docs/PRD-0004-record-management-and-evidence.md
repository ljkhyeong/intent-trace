...
- 검증별 `검증 시각과 해시`를 펼치면 실행 시각·검증 스냅샷 해시·출력 해시를 확인한다. 기록 정보의 스냅샷 해시와 비교할 수 있도록 각 검증의 원래 값을 표시한다.
- 기록 상세에서 Markdown 파일을 저장한다. `GET /records/{UUID}/markdown`은 단건 조회와 같은 권한을 적용하며 저장된 현재 기록을 내보낸다. 공개·대체나 GitHub 게시를 수행하지 않는다.
- 공유용 Markdown에는 검증별 종료 코드·실행 시각(UTC)·검증 스냅샷 해시·출력 해시를 표시한다. 다른 스냅샷의 결과도 실행 정보를 유지하며 현재 코드의 검증으로 표시하지 않는다.
- 로그인 전에는 본문을 표시하지 않는다. GitHub 로그인 완료 후 원래 기록 또는 검색으로 돌아온다. 로그인 복귀 주소와 브라우저 전용 `itb_` 인증 규칙은 ADR-0009를 따른다.
...
- 생성·초안 수정·후속 초안은 `relativePath`와 `relatedPath`에서 `./`·중복 `/`·끝 `/`을 제거한 뒤 서로 연결되는지 확인한다. 같은 경로의 표기 차이는 허용하고 반대쪽 코드 근거가 없으면 거부한다.
...
- 실행 전후 HEAD가 같고 Git이 보고하는 수정·미추적 파일이 없어야 한다. 서브모듈도 `.gitmodules`·로컬 Git의 변경 숨김 설정과 무관하게 검사한다. 변경되면 저장 가능한 검증 JSON을 반환하지 않는다. Git이 무시하는 파일, 실행 도중 변경 후 복원, 빌드 환경 전체의 동일성까지 증명하지는 않는다.
...
- PR 기록 화면에서 표시 중인 전체 HEAD를 기준으로 CI 결과 화면에 이동한다. 사용자가 커밋 해시를 다시 입력할 필요가 없으며 PR을 다시 조회하기 전에는 링크의 커밋을 바꾸지 않는다.
...
- 결과의 `당시 코드 열기`는 원본 커밋·경로·줄을 가리킨다. 현재 줄 범위를 확인한 결과에만 `조회한 커밋의 코드 열기`를 표시한다. 링크는 전체 커밋 해시를 사용하고 이름 변경·줄 이동을 반영하며, 추가 GitHub 조회는 하지 않는다.
- 기록 화면의 코드 확인 링크는 `/records/{UUID}/evidence`를 연다. 확인 시각·기록 버전·커밋·스냅샷·근거별 결과를 보여주고 테스트 실행 증명과 구분한다. 일반 기록 열람은 추가 원격 확인을 실행하지 않는다.
- 검색에서 연 기록은 원본·대체 기록, 원본 비교·코드 확인·변경 이력을 거쳐도 검색어·필터·검색 페이지를 유지한다. 이력 페이지와 비교 필터는 검색 조건과 구분하며, 코드 확인 불가 화면에서도 같은 기록으로 돌아간다.
...
- 브라우저 쿠키를 REST·MCP 인증에 사용하지 않는다. [브라우저 세션 규칙](ADR-0009-browser-record-access.md)을 따른다.
...
- 서버의 코드 HTTP 호출 수 설정은 7~200이다. 7회는 빈 캐시에서 근거 하나를 확인하는 데 필요한 최대 호출 수이며, 이보다 작은 설정은 서버 시작 시 거부한다. 기본 30초·40회와 시간 상한 40초는 유지한다.
- REST·MCP history 응답에 `resumeBlocked`를 추가한다. 현재 요청의 첫 미완료 근거도 처리하지 못하고 `TIME_LIMIT` 또는 `CALL_LIMIT`에 도달하면 true, 나머지는 false다. 결과가 없다는 이유만으로 true로 표시하지 않는다.
- `resumeBlocked=true`이면 `complete=false`이며 미확인 근거의 `nextCursor`를 유지한다. 클라이언트는 같은 커서를 자동 반복하지 않는다. 서버 조회 제한·GitHub 지연을 확인하고 조치한 뒤 같은 커서로 다시 요청한다. `CANCELLED`도 사용자가 재개를 요청한 뒤 다시 조회한다.
- 웹은 원인 확인 안내와 수동 재조회 링크를 표시한다. 정상적으로 일부 근거를 처리한 중단 응답은 기존 이어 읽기 안내를 유지한다. 원격 응답이나 인증 정보를 요청 사이에 추가로 저장하지 않는다.
