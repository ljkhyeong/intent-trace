...
- [config.env](../../deploy/k3s/config.env), [.env.example](../../deploy/k3s/.env.example): 앱 설정과 GitHub 비밀값 예시
- [database.env](../../deploy/k3s/database.env), [.env.database.example](../../deploy/k3s/.env.database.example): DB 접속 설정과 계정 예시
...
1. 앱·DB 비밀값 예시를 각각 복사하고 권한을 제한한다. 이미 준비한 파일이 있으면 덮어쓰지 않고 편집한다.

   ```bash
   cp -n deploy/k3s/.env.example deploy/k3s/.env.secrets
   cp -n deploy/k3s/.env.database.example deploy/k3s/.env.database.secrets
   chmod 600 deploy/k3s/.env.secrets deploy/k3s/.env.database.secrets
   ```

2. `.env.secrets`의 GitHub App client ID·client secret·Base64 private key·웹훅 secret과 `.env.database.secrets`의 DB 계정·비밀번호를 실제 값으로 바꾼다. 예시 값으로는 로그인과 게시가 동작하지 않는다. 두 비밀값 파일은 Git에서 제외한다. DB 이름을 바꾸면 `database.env`의 이름과 JDBC URL 끝의 DB 이름을 함께 맞춘다. 기존 PostgreSQL PVC의 비밀번호를 바꿀 때는 환경 파일 변경과 별도로 DB 계정 비밀번호도 변경해야 한다.
...
기존 공용 환경 파일을 사용했다면 DB 이름·URL은 `config.env`에서 `database.env`로, DB 계정·비밀번호는 `.env.secrets`에서 `.env.database.secrets`로 옮긴다. 현재 값을 유지하고 예시 값으로 덮어쓰지 않는다. 처음 분리해서 적용할 때는 참조 이름이 바뀌므로 앱과 DB가 모두 교체된다.
...
검증기는 Kubernetes 1.35 스키마와 설정별 교체 범위를 검사한다. 앱 설정·GitHub 비밀값 변경은 앱에만, DB 설정·비밀번호 변경은 앱과 DB 양쪽에 반영되는지 렌더링 결과로 확인한다. 실제 클러스터의 저장소·Ingress·인증서·네트워크 연결까지 확인하는 명령은 아니다. `kubectl kustomize deploy/k3s` 출력에는 Secret이 Base64로 포함되므로 로그·이슈·채팅에 붙이지 않는다. Base64는 암호화가 아니다.
...
ConfigMap·Secret 이름에는 내용 해시가 붙는다. 앱과 DB 설정을 분리하므로 GitHub 설정만 바꾸면 앱 Pod만 교체된다. DB 설정이나 계정을 바꾸면 양쪽 Pod가 교체되므로 점검 시간에 적용한다. 이전 Secret을 정리할 때는 현재 앱·PostgreSQL이 참조하는 이름을 먼저 확인한다. DB PVC는 앱 배포와 함께 삭제하지 않는다.
