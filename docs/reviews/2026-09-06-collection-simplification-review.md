...
기준: `e7f1a8a`, 검토 시작 시 미커밋 변경 없음. 앞선 JDBC 일괄 조회·이력 재개·세션 종료 개선을 제외하고 서버의 Git 코드 조회·기록 비교·게시·세션 처리를 확인했다. 아래 설명은 검토 당시 상태이며 이후 구현·검증은 반영 결과에 정리했다.
...
대규모 구조 변경이나 추가로 삭제할 권한·도메인·동시성 검증은 검토 범위에서 찾지 못했다. 두 항목은 우선순위가 낮은 소규모 개선으로 판단했다.

최초 검토에서는 코드와 표준 라이브러리 소스만 확인했다. 당시에는 제품 코드·테스트·설정·의존성을 변경하지 않아 테스트를 다시 실행하지 않았다. 문서의 로컬 링크와 `git diff --check`를 확인했다.

## 반영 결과

- 이름 변경 판별의 `count { ... } == 1`을 `singleOrNull { ... } != null`로 바꿨다. 기존 이력 테스트에서 이전·현재 트리 어느 쪽이든 같은 blob이 중복되면 이름 변경으로 표시하지 않는 것을 확인했다.
- Git 트리는 경로별 Map을 바로 만들고 원본과 크기를 비교한다. 중복 경로와 잘못된 객체 형식이 함께 있을 때도 중복 오류를 먼저 반환하는 테스트를 보완했다.
- 기록 비교는 기존 인덱스 Map의 크기로 중복을 확인한다. 원본·후속 기록의 중복은 모두 `AMBIGUOUS`로 반환하고 기존 출처 변경·순서 이동 판별도 유지한다.
- 검증 대상은 `3ac68ac`에 위 구현과 테스트 변경을 적용한 상태다. `./gradlew focusedTest --tests '*RecordEvidenceIntegrationTest' --tests '*GitHubEvidenceClientTest' --tests '*DraftManagementIntegrationTest'` 13개, 이후 `./gradlew test` 171개가 통과했다. 실패·건너뜀은 없으며 부분 테스트는 전체에 포함된다. 검증 후 제품 코드·테스트는 변경하지 않았다.
- JDBC·DB 스키마·IDE 구현은 변경하지 않아 PostgreSQL·IDE 검증은 다시 실행하지 않았다. 외부 게시·배포는 수행하지 않았고 지연 시간 개선율도 측정하지 않았다.
