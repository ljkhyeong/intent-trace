...
3. 로그인 완료 화면의 `its_` 세션 토큰을 `IntentTrace 세션 연결`에 입력한다. 토큰은 IntelliJ PasswordSafe에 저장된다. 설정의 `로그인 확인`으로 세션 유효 여부와 GitHub 계정을 확인할 수 있다.
...
- 조회·연결 확인·세션 해제의 `429`는 서버가 `Retry-After`에 보낸 초 단위 대기 시간을 표시한다. 값이 없거나 유효한 숫자가 아니면 잠시 후 재시도를 안내한다. 자동 재시도하지 않으며 세션 해제 실패 시 저장된 토큰을 유지한다.
