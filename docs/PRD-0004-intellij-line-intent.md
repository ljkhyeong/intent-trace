...
3. 로그인 완료 화면의 `its_` 세션 토큰을 `IntentTrace 세션 연결`에 입력한다. 서버가 계정을 확인하면 IntelliJ PasswordSafe에 저장하고 GitHub 계정을 표시한다. 이후에는 설정의 `로그인 확인`으로 저장 세션의 상태를 확인할 수 있다.
...
8. 요청·구현 결정과 이유·등록된 검증 결과·관련 코드·남은 질문을 읽기 전용 창으로 보여준다. 구현 결정에는 출처를 함께 표시한다.
   기록을 고르고 `선택 기록 열기`를 누르면 상세에서 원래 커밋·당시 코드·원본·대체 기록으로 이동할 수 있다. 결과 유무와 관계없이 `이 파일의 과거 기록 보기`로 PRD-0005의 파일 이력을 연다. 과거 기록을 현재 줄의 근거로 자동 연결하지 않는다.
   `웹에서 줄 이동·이름 변경 찾기`는 조회한 서버의 웹 이력 화면을 같은 저장소·전체 커밋·파일·줄로 연다. 웹 로그인 후 서버의 기존 코드 비교로 확인한다. 결과가 없어도 같은 창을 표시하며 기록 선택과 상세 열기만 비활성화한다.
...
- 적용한 주소는 IDE 재시작 없이 새로 여는 조회와 연결 작업에 반영한다. 열린 결과에서 이어가는 상세·과거 기록 조회는 조회 당시 서버를 유지한다. 적용 전 취소한 내용과 연결 확인만 실행한 주소는 저장하지 않는다.
- 서버를 바꿔도 다른 서버의 PasswordSafe 세션을 전달하지 않는다. 환경 변수 세션도 환경 변수로 지정한 서버에만 사용한다.
- 세션 연결은 입력한 토큰으로 서버의 계정을 확인한 뒤 해당 서버의 저장 세션을 교체한다. 인증 거부·호출 제한·서버 장애·잘못된 응답이면 저장하지 않고 기존 세션을 유지한다. 환경 변수 세션은 변경하지 않는다.
- 저장 세션이 없으면 서버에 폐기를 요청하지 않고 삭제할 세션이 없다고 안내한다. 삭제 여부와 관계없이 해당 서버의 환경 변수 세션이 남아 있으면 계속 사용된다고 표시한다.
...
- 로그인 확인은 입력한 서버에 대응하는 세션만 보내 서버가 확인한 GitHub 계정을 표시한다. 세션 누락은 요청 전에 안내하고 만료·권한 거부·API 미지원은 구분한다. 확인만으로 서버 주소나 세션을 저장하지 않는다.
...
- 조회·연결 확인·세션 연결·해제의 `429`는 서버가 `Retry-After`에 보낸 초 단위 대기 시간을 표시한다. 값이 없거나 유효한 숫자가 아니면 잠시 후 재시도를 안내한다. 자동 재시도하지 않으며 세션 연결·해제 실패 시 저장된 토큰을 유지한다.
- 조회 결과에서 구현 결정의 출처, 검증 결과의 수집 출처와 적용 대상, 남은 질문을 구분한다. 기록의 변경 후 커밋과 조회 커밋이 다르면 `다른 커밋의 결과`로 표시하며 스냅샷 불일치로 단정하지 않는다.
- 관련 코드의 변경 전·후를 표시하고 상세의 코드 링크는 해당 커밋으로 연다. `BASE`는 `baseRevision`, `TARGET`은 `targetRevision`을 사용하며 한쪽 커밋이 없을 때 다른 쪽으로 대신 연결하지 않는다.
