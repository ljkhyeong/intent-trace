...
IntelliJ 편집기에서 현재 줄의 공개 기록을 조회합니다. 플러그인은 저장소·커밋·파일·줄을 읽고, 기록 공개 범위와 저장소 권한은 서버가 확인합니다.
...
- PasswordSafe 세션 폐기·삭제와 환경 변수 세션 사용 안내
...
- 조회 결과에서 구현 결정의 출처, 기록과 검증 스냅샷의 일치 여부, 남은 질문을 구분한다.
