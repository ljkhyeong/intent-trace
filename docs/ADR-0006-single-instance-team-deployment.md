...
- backup은 최종 경로와 같은 디렉터리의 임시 파일에 custom-format `pg_dump`를 만들고 권한을 `0600`으로 제한한다. 완성한 임시 파일은 하드 링크로 최종 경로를 배타적으로 확보한 뒤 공개한다. 같은 경로의 다른 실행이 먼저 완료됐으면 실패한 실행은 자신의 임시 파일만 삭제하고 기존 backup을 유지한다. `HUP`·`INT`·`TERM`을 받으면 성공 경로를 계속 실행하지 않고 신호별 종료 코드로 끝낸 뒤 자신의 임시 파일을 정리한다.
