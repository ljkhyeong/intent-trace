...
기준: IntentTrace 0.12.0 · 2026-09-05

## 로컬 설치 파일

Node.js 22 이상, npm과 macOS 또는 Linux에서 실행한다. 고정 의존성을 먼저 설치한다.
...
`build/zed-release/intent-trace-zed-0.12.0.tgz`와 같은 이름의 `.sha256` 파일을 만든다. Node 의존성이 포함되어 빈 npm 캐시에서도 오프라인 설치할 수 있다. `--output 폴더`로 생성 위치를 바꿀 수 있다. 이 파일은 로컬 설치용이며 npm 공개 게시가 비활성화되어 있다.

설치·버전 변경·제거는 [패키지 사용 안내](../../clients/zed/README.md)를 따른다. 생성 파일에는 실제 사용자 설정·세션·서버 소스·테스트가 포함되지 않는다. 패키지를 만든 시점의 Node 의존성을 사용하므로 의존성을 바꿀 때는 고정 목록과 검증을 함께 갱신한다.
